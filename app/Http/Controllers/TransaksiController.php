...
public function store(Request $request)
    {
        ...
        $no = sprintf("%04d", $no);
        $discount = 0;
        if ($request->pelanggan_id && $total >= 250000) {
            $discount = $total * 0.2;
        }
    ...
    public function restoreStock(Penjualan $transaksi)
    {
    ...
    return back()->with('restore', 'success');
    }

    public function destroy(Request $request, Penjualan $transaksi)
    {
        $transaksi->update(['status'=>'batal']);
        return redirect()->route('transaksi.restore-stock', [
            'transaksi' => $transaksi->id
        ])->with('destroy','success');
    }
